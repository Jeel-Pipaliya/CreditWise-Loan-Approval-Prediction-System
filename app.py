...
with st.form("loan_form"):
    ...
    age            = c1.number_input("Age",             min_value=18, max_value=75, value=32)
    gender         = c2.selectbox("Gender",             ["Male", "Female"], index=1)
    marital_status = c3.selectbox("Marital Status",     ["Married", "Single"], index=0)
    dependents     = c4.number_input("Dependents",      min_value=0, max_value=10, value=2)

    education_level   = c1.selectbox("Education Level",   ["Graduate", "Not Graduate"], index=1)
    employment_status = c2.selectbox("Employment Status", ["Salaried", "Self-employed", "Contract", "Unemployed"], index=0)
    employer_category = c3.selectbox("Employer Category", ["Government", "Private", "MNC", "Business"], index=2)
    property_area     = c4.selectbox("Property Area",     ["Urban", "Semiurban", "Rural"], index=0)

    # ── Financial Information ─────────────────────────────────────────────────
    st.markdown('<div class="section-title">💰 Financial Information</div>', unsafe_allow_html=True)
    c1, c2, c3, c4 = st.columns(4)
    applicant_income    = c1.number_input("Applicant Income (₹)",    min_value=0, value=18023, step=1000)
    coapplicant_income  = c2.number_input("Co-Applicant Income (₹)", min_value=0, value=4033,  step=1000)
    savings             = c3.number_input("Savings Balance (₹)",     min_value=0, value=18244, step=5000)
    collateral_value    = c4.number_input("Collateral Value (₹)",    min_value=0, value=40004, step=10000)

    credit_score    = c1.number_input("Credit Score",      min_value=300, max_value=900, value=688)
    existing_loans  = c2.number_input("Existing Loans",    min_value=0,   max_value=10,  value=1)
    dti_ratio       = c3.number_input("DTI Ratio",         min_value=0.0, max_value=1.0, value=0.13, step=0.01)

    # ── Loan Details ──────────────────────────────────────────────────────────
    st.markdown('<div class="section-title">🏦 Loan Details</div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    loan_amount  = c1.number_input("Loan Amount (₹)",    min_value=10000, value=10415, step=10000)
    loan_term    = c2.number_input("Loan Term (months)", min_value=6,     value=12,    step=6)
    loan_purpose = c3.selectbox("Loan Purpose",          ["Home", "Education", "Personal", "Business"], index=1)
